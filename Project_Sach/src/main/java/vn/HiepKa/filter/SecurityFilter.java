...
public class SecurityFilter implements Filter {
    ...
    private static final Pattern SQL_INJECTION_PATTERN = Pattern.compile("(['\";\\-]|(?i)(select|insert|update|delete|drop|truncate|union|exec|sp_executesql|xp_cmdshell))");
    private static final Pattern SSTI_PATTERN = Pattern.compile("(?i)(\\{\\{.*?\\}\\}|\\$\\{.*?\\}|#\\{.*?\\})");
    private static final Pattern XSS_PATTERN = Pattern.compile("(?i)(<script.*?>.*?</script>|javascript:|onerror=|<.*?on\\w+=)");
    ...
    private boolean isMalicious(String input) {
        return SQL_INJECTION_PATTERN.matcher(input).find() || 
               SSTI_PATTERN.matcher(input).find() || 
